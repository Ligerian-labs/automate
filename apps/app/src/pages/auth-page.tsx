...
export function AuthPage({ mode }: { mode: "login" | "register" }) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await apiFetch<{ token: string }>(path, { method: "POST", body: JSON.stringify(body) }, false);
  ...
  return (
    <div className="grid min-h-screen place-items-center bg-[var(--bg-primary)] p-4">
      ...
      <div className="flex w-full max-w-[420px] flex-col rounded-2xl border border-[var(--divider)] bg-[var(--bg-surface)] p-10 shadow-2xl" style={{ gap: 32 }}>
        ...
        <div className="flex flex-col items-center" style={{ gap: 10 }}>
          <div className="flex items-center" style={{ gap: 10 }}>
            ...
            <span className="text-xl font-bold" style={{ fontFamily: "var(--font-mono)" }}>stepIQ</span>
          </div>
          {/* Header — gap 8 */}
        ...
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          {mode === "register" ? (
            <div className="flex flex-col gap-1.5">
              <span className="text-[13px] font-medium text-[var(--text-secondary)]">Name</span>
              <input
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between">
              ...
              {mode === "login" ? (
                <button type="button" className="text-xs font-medium text-[var(--accent)]">Forgot?</button>
              ) : null}
            </div>
            <input
              className="w-full rounded-lg border border-[var(--divider)] bg-[var(--bg-inset)] px-4 py-3 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:border-[var(--accent)] focus:outline-none"
        ...
        <p className="text-center text-[13px] text-[var(--text-tertiary)]">
          ...
          <a className="font-semibold text-[var(--accent)] hover:underline" href={mode === "login" ? "/register" : "/login"}>
