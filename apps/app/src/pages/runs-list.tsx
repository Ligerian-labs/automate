...
export function RunsListPage() {
  ...
  const runsQ = useQuery({ queryKey: ["runs"], queryFn: () => apiFetch<RunRecord[]>("/api/runs?limit=50") });
    ...
    <AppShell title="Runs" subtitle="View all pipeline execution history">
      <section className="overflow-hidden rounded-xl border border-[var(--divider)] bg-[var(--bg-surface)]">
        <div
          ...
          style={{ gridTemplateColumns: "minmax(200px,1fr) 120px 120px 100px 100px 120px", fontFamily: "var(--font-mono)" }}
        >
          <span className="text-[11px] font-semibold uppercase tracking-[1px] text-[var(--text-tertiary)]">Run ID</span>
          <span className="text-[11px] font-semibold uppercase tracking-[1px] text-[var(--text-tertiary)]">Status</span>
          <span className="text-[11px] font-semibold uppercase tracking-[1px] text-[var(--text-tertiary)]">Trigger</span>
          <span className="text-[11px] font-semibold uppercase tracking-[1px] text-[var(--text-tertiary)]">Steps</span>
          <span className="text-[11px] font-semibold uppercase tracking-[1px] text-[var(--text-tertiary)]">Tokens</span>
          <span className="text-right text-[11px] font-semibold uppercase tracking-[1px] text-[var(--text-tertiary)]">Cost</span>
        </div>

        {runsQ.isLoading ? <p className="p-5 text-sm text-[var(--text-tertiary)]">Loading runs...</p> : null}
        {runsQ.isError ? (
          <p className="p-5 text-sm text-red-300">{runsQ.error instanceof Error ? runsQ.error.message : "Failed to load"}</p>
        ) : null}
          ...
          {(runsQ.data ?? []).map((run) => {
            ...
            return (
              <button
                ...
                style={{ gridTemplateColumns: "minmax(200px,1fr) 120px 120px 100px 100px 120px" }}
              ...
              >
                <div className="flex flex-col gap-0.5">
                  ...
                  <p className="text-[11px] text-[var(--text-tertiary)]">
                    {run.createdAt || run.created_at ? timeAgo(new Date(run.createdAt || run.created_at || "")) : "-"}
                  </p>
                </div>
                <div>
                ...
                <div className="text-right text-[13px] text-[var(--text-secondary)]" style={{ fontFamily: "var(--font-mono)" }}>
                  €{((run.totalCostCents ?? run.total_cost_cents ?? 0) / 100).toFixed(2)}
                </div>
              </button>
            );
          })}
          {(runsQ.data ?? []).length === 0 && !runsQ.isLoading ? (
            <p className="p-8 text-center text-sm text-[var(--text-tertiary)]">No runs yet — execute a pipeline to see results here.</p>
          ) : null}
        </div>
      </section>
    </AppShell>
